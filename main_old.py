...
source = 0
biometric = 0
#base_url = "http://localhost/timetracker/"
...
class register:
    ...
    def __init__(self):
        ...
        self.new_item.add_separator()
        self.menu.add_cascade(label='Settings', menu=self.new_item)
        ...
        self.quitWindow = tk.Button(text="Login", command=self.submit  ,fg="white",bg="#1d2736" ,font="MSGothic 18 bold")
        self.quitWindow.place(x=180, y=200)
        self.clearWindow = tk.Button(text=" Exit ", command=self.ExitApplication  ,fg="white",bg="#1d2736" ,font="MSGothic 18 bold")
    ...
    def camera(self):
        ...
        self.txt1 = tk.Spinbox(self.top,values=('Cam1','Cam2','Cam3'),textvariable=self.val,width=15  ,fg="#1d2736",font=('times', 15, ' bold '))
        self.txt1.place(x=10 ,y= 150)
        self.submit=tk.Button(self.top,text='Submit', command=self.channelno,fg="white",bg="#1d2736" ,font="MSGothic 13 bold")
    ...
    def srccapture(self):
        ...
        self.txt1 = tk.Spinbox(self.top,values=('05','10','15','30'),textvariable=self.val,width=15  ,fg="#1d2736",font=('times', 15, ' bold '))
        self.txt1.place(x=10 ,y= 90)
        self.submit=tk.Button(self.top,text='Submit', command=self.timeslot,fg="white",bg="#1d2736" ,font="MSGothic 13 bold")
    ...
    def ExitApplication(self):
        ...
        if MsgBox == 'yes':
            ...
            self.tk.messagebox.showinfo('Return','You will now return to the application screen')
...
def login_dynamic():
    class user_login:
        ...
        def __init__(self):
            ...
            self.label_msg1 = canvas.create_text((150, 150), text="Employee Id",justify = tk.CENTER, font="MSGothic 16 bold", fill="white")
            self.txt = tk.Entry(width=26  ,fg="#1d2736",font=('times', 15, ' bold '))
            self.txt.place(x=220, y=138)
            self.quitWindow1 = tk.Button(text="OTP", command=self.submit1  ,fg="white",bg="#1d2736" ,font="MSGothic 15 bold")
            self.quitWindow1.place(x=220, y=190)
            self.quitWindow = tk.Button(text="FACE", command=self.submit  ,fg="white",bg="#1d2736" ,font="MSGothic 15 bold")
            self.quitWindow.place(x=320, y=190)
            self.clearWindow = tk.Button(text="EXIT", command=self.ExitApplication  ,fg="white",bg="#1d2736" ,font="MSGothic 15 bold")
            self.clearWindow.place(x=420, y=190)
            self.root.mainloop()
        ...
        def submit1(self):
            ...
            if json_data_load['status'] == 0:
                ...
                post_data = {'to_email': useremail, 'to_name': useremail, 'to_secret_key': OTP, 'page': 'otpverify', 'subject': 'OTP Verification'}
                response = requests.request("POST", url, data=post_data, files=files)
                self.root.destroy()
...
def task(y,kname,kencod,kid,lic_key,projectid,finalarray):
    ...
    class project:
        ...
        def __init__(self,y,kname,kencod,kid,lic_key,projectid,finalarray):
            ...
            self.bg_img = tk.PhotoImage(file="deer_decode.gif")
            self.bg_label = canvas.create_image((0,0), image=self.bg_img, anchor=tk.N+tk.W)
            self.label_msg1 = canvas.create_text((150, 150), text="Task Name",justify = tk.CENTER, font="MSGothic 16 bold", fill="white")
            self.txt1 = tk.Spinbox(self.root,values= self.values,textvariable=self.val,width=20  ,fg="#1d2736",font=('times', 15, ' bold '))
            self.txt1.place(x=220 ,y= 138)
            
            
            self.quitWindow = tk.Button(text="Submit", command=self.submit  ,fg="white",bg="#1d2736" ,font="MSGothic 13 bold")
            self.quitWindow.place(x=230, y=230)
            self.clearWindow = tk.Button(text="Exit", command=self.ExitApplication  ,fg="white",bg="#1d2736" ,font="MSGothic 13 bold")
...
def project(y,kname,kencod,kid,lic_key,finalarray):
    ...
    class project:
        ...
        def __init__(self,y,kname,kencod,kid,lic_key,finalarray):
            ...
            self.bg_img = tk.PhotoImage(file="deer_decode.gif")
            self.bg_label = canvas.create_image((0,0), image=self.bg_img, anchor=tk.N+tk.W)
            self.label_msg1 = canvas.create_text((150, 150), text="Project Name",justify = tk.CENTER, font="MSGothic 16 bold", fill="white")
            self.txt1 = tk.Spinbox(self.root,values= self.values,textvariable=self.val,width=20  ,fg="#1d2736",font=('times', 15, ' bold '))
            self.txt1.place(x=220 ,y= 138)
...
def sessio_dynamic1(name,kname,kencod,kid,lic_key,projectid,task):
    class wfh:
        ...
        def __init__(self,name,kname,kencod,kid,lic_key,projectid,task):
            ...
            self.screen = tk.Label(self.root, text="LATEST SCREENSHOT",fg="white",bg="#1d2736" ,font="MSGothic 14 bold")
            ...
            if not self._running :
                ...
                for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
                    ...
                    if name == self.name:
                        ...
                        self.kimg1=faceimg
                        print("inside start face = ",faceimg)
                        self.pcount+=1
                        print("inserting")
                        status = self.insert(time11,date)
                        print(status)
        ...
        def insert(self,time,date):
            ...
            if self.connect():
                ...
                data = {"useruniqueid": str(user_unique_id),
                    ...
                    "faceshot": str(file_loc1), "duration": str(duration), "createddate": str(date), "captureddate": str(date), "requesttype": str("face")}
...
def art(name,kid,lic_key,OTP,finalarray):
    class wfh:
        ...
        def __init__(self,name,kid,lic_key,OTP,finalarray):
            ...
            self.bg_label = canvas.create_image((0,0), image=self.bg_img, anchor=tk.N+tk.W)
            #self.label_msg = canvas.create_text((150, 40), text="AIRFACE TimeTracker",justify = tk.CENTER, font="ALGERIAN 18 bold", fill="white")
            self.label_msg1 = canvas.create_text((150,150), text="ENTER OTP",justify = tk.CENTER, font="MSGothic 16 bold", fill="white")
            self.txt = tk.Entry(width=26  ,fg="#1d2736",font=('times', 15, ' bold '))
            self.txt.place(x=220, y=138)
            self.quitWindow1 = tk.Button(text="EXIT", command=self.ExitApplication ,fg="white",bg="#1d2736" ,font="MSGothic 13 bold")
            self.quitWindow1.place(x=390, y=190)
            
            self.clearWindow = tk.Button(text="SUBMIT", command=self.submit  ,fg="white",bg="#1d2736" ,font="MSGothic 13 bold")
        ...
        def insert(self,time,date):
            ...
            if self.connect():
                ...
                data = {"useruniqueid": str(empid),
                    ...
                    "projectid": str(projectname),
                    "faceshot": str(""),"duration": str(duration), "createddate": str(date), "captureddate": str(date), "requesttype": str("otp"), "tasktitle": str(self.task)}
                X=requests.post(url, data)
                #print("======================done==============")
                print(X)
                #print("======================done==============")
                    ...
                    for path,dirs,files in os.walk(start_path):
                        for filename in files:
                            ...
                            data = {"useruniqueid": str(empid),
                                "useremail": str(user_unique_id), "datetime": str(db_time_format), "screenshot": str(file_loc),
                                "projectid": str(projectname),
                                "faceshot": str(""),"duration": str(duration), "createddate": str(date), "captureddate": str(date), "requesttype": str("otp")}
                            X=requests.post(url, data)
